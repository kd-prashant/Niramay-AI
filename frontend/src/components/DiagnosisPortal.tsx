      ...
      <div className="mb-12 md:mb-20 overflow-x-auto pb-6 md:pb-0 scrollbar-hide snap-x snap-mandatory">
        ...
        >
          <div className="flex items-center gap-4 px-4 snap-center">
              ...
              <div>
                  <span className="block text-[8px] md:text-[10px] font-black uppercase tracking-widest opacity-30 mb-1">Live Location</span>
                  <span className="text-sm md:text-lg font-black text-white whitespace-nowrap">{t('loc')}</span>
              </div>
          </div>
          ...
          <div className="flex items-center gap-4 px-4 md:border-l border-white/5 snap-center">
              ...
              <div>
                  <span className="block text-[8px] md:text-[10px] font-black uppercase tracking-widest opacity-30 mb-1">{t('advisory')}</span>
                  <span className="text-sm md:text-lg font-black text-blue-500 whitespace-nowrap">{t('risk')}</span>
              </div>
          </div>
          ...
          <div className="flex items-center gap-4 px-4 md:border-l border-white/5 snap-center">
              ...
              <div>
                  <span className="block text-[8px] md:text-[10px] font-black uppercase tracking-widest opacity-30 mb-1">{t('health_index')}</span>
                  <span className="text-sm md:text-lg font-black text-emerald-500 uppercase whitespace-nowrap">{t('health_status')}</span>
              </div>
          </div>
        </motion.div>
      </div>
          ...
          <AnimatePresence mode="wait">
            {!result && !isScanning && (
              ...
              >
                    ...
                    <motion.button
                      ...
                      onClick={startScan}
                      disabled={!image}
                      className={`col-span-2 py-8 rounded-[35px] font-black text-2xl transition-all flex items-center justify-center gap-4 shadow-2xl ${
                        !image ? 'bg-zinc-800 text-white/20' : 'bg-white text-black active:bg-veridian-500 active:text-white'
                      }`}
                    >
            ...
            {result && (
              <motion.div 
                ...
                className="space-y-6"
              >
                <div className={`p-10 rounded-[50px] transition-all duration-700 ${
                  result.prediction.severity === 'None' ? 'bg-veridian-600' : 
                  result.prediction.severity === 'Critical' ? 'bg-red-600 shadow-[0_40px_100px_rgba(239,68,68,0.4)]' : 'bg-amber-600 shadow-[0_40px_100px_rgba(245,158,11,0.4)]'
                    ...
                    <h2 className="text-4xl md:text-5xl font-black mb-3 tracking-tighter leading-none">{result.prediction.common_name}</h2>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-black/10 rounded-3xl p-8 border border-white/10 text-center">
                            ...
                            <span className="text-[10px] uppercase font-black opacity-40">{t('conf')}</span>
                        </div>
                        <div className="bg-black/10 rounded-3xl p-8 border border-white/10 text-center">
                            <span className="block text-4xl font-black">{result.prediction.severity}</span>
                            <span className="text-[10px] uppercase font-black opacity-40">{t('risk_level')}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="p-10 bg-zinc-900 rounded-[50px] border border-white/10 shadow-xl">
                        <h4 className="text-[10px] font-black uppercase tracking-[0.3em] mb-8 text-blue-500">{t('treatment')}</h4>
                        <div className="flex flex-wrap gap-3">
                            {result.recommendations.treatment.medicines.map((m: string, i: number) => (
                                <span key={i} className="px-5 py-3 bg-black text-white text-[10px] font-black rounded-2xl border border-white/10">{m}</span>
                            ))}
                        </div>
                    </div>
                    <div className="p-12 bg-white text-black rounded-[50px] shadow-2xl">
                         <h4 className="text-[10px] font-black uppercase tracking-widest mb-8 text-veridian-600">{t('action_plan')}</h4>
                        <div className="space-y-6">
                            {result.recommendations.treatment.action_plan.map((step: string, i: number) => (
                                <div key={i} className="flex gap-6 items-start">
                                    ...
                                    <p className="text-sm font-bold text-black/60 leading-relaxed">{step}</p>
                                </div>
                            ))}
                        </div>
                    </div>
